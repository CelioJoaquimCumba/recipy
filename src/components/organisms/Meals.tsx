...
export const Meals = ({meals}: meals) => {
    return (
        <div className="flex gap-2 self-stretch overflow-auto min-h-[18.75rem] justify-start items-center ">
            {meals &&meals.length === 0 && <p className="text-base leading-6 font-normal text-gray-500 w-full text-center">No meals available :( Try another category</p>}
            {meals && meals.map((meal:meal | undefined) => {
                if(meal) return (
